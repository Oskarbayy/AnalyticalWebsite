let variants = ["Normal", "Golden", "Rainbow"];
...
function CreateButton(pet) {
    ...
    main.addEventListener("click", function () {
        window.open(
            `PetDetails.html?petId=${pet.configName}&variant=${pet.Variant}&shiny=${pet.sh}`,
            "_self"
        );
    });
    ...
    main.innerHTML = `
        <div class="ImageDiv"> 
            ...
            ${pet.sh === true ? `<div class="shiny-overlay"></div>` : ""}
        </div>
...
async function GetData() {
    ...
    // Helper Function
    const isShiny = (pet) => pet.configData.sh === true;
    const isVariant = (pet, pt) =>
        pet.configData.pt === pt || (!pet.configData.pt && pt == 0);
    ...
    for (let pet of hugePets) {
        ...
        for (let pt = 0; pt < 3; pt++) {
            // non shiny variant
            let rapPet = rapData.find(
                (p) =>
                    p.configData.id === petId && isVariant(p, pt) && !isShiny(p)
            );
            let existsPet = existsData.find(
                (p) =>
                    p.configData.id === petId && isVariant(p, pt) && !isShiny(p)
            );
            ...
            // shiny variant
            let rapPetShiny = rapData.find(
                (p) =>
                    p.configData.id === petId && isVariant(p, pt) && isShiny(p)
            );
            let existsPetShiny = existsData.find(
                (p) =>
                    p.configData.id === petId && isVariant(p, pt) && isShiny(p)
            );

            if (rapPetShiny && existsPetShiny) {
                let normalPet = {
                    ...pet,
                    Variant: variants[pt],
                    thumbnail: pet.configData.thumbnail,
                    RAP: rapPetShiny.value,
                    Exists: existsPetShiny.value,
                    sh: true,
                };
                allPets.push(normalPet);
            }
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function filterAndSortPets() {
        ...
        const searchTerms = searchTerm
            .split(" ")
            .filter((term) => term.trim() !== ""); // Split into individual words and remove empty entries
            ...
            return (
                terms.reduce((totalScore, term) => {
                    ...
                    for (let i = 0; i <= name.length - term.length; i++) {
                        ...
                        termScore = Math.min(
                            termScore,
                            getLevenshteinDistance(part, term)
                        );
                    }
        ...
        let filteredPets = allPets.filter((pet) => {
            ...
            return (
                (searchTerms.length === 0 ||
                    calculateFuzzyMatchScore(name, searchTerms) > 0.5) &&
                (isNormal || isGolden || isRainbow)
            );
        });
        ...
        filteredPets.sort((a, b) => {
            ...
            const countOccurrences = (name, char) =>
                name.split(char).length - 1;
